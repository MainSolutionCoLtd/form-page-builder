...
export function SaveAsModal({ chrome, suggestedName, templateState, onSave, onClose }: SaveAsModalProps) {
  ...
  return (
    <div style={styles.modalOverlay} onClick={onClose}>
      <div ref={panelRef} style={{ ...styles.modal, maxWidth: 380 }} role="dialog" aria-modal="true" aria-label={chrome.saveForm} onClick={(e) => e.stopPropagation()}>
        <div style={styles.modalHeader}><span style={{ fontWeight: 600, fontSize: 14 }}>{chrome.saveForm}</span><button type="button" style={styles.iconBtn} onClick={onClose}><X size={16} /></button></div>
        <div style={{ padding: 16 }}>
          ...
          <button type="button" style={{ ...styles.primaryBtn, width: "100%", justifyContent: "center", marginTop: 14 }} disabled={!canSave} onClick={() => onSave(name.trim())}>
