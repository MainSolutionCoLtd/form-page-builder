...
export function TemplatesModal({ chrome, savedForms, currentFormId, manage, templateState, onOpen, onDelete, onClose }: TemplatesModalProps) {
  ...
  return (
    <div style={styles.modalOverlay} onClick={onClose}>
      <div style={styles.modal} role="dialog" aria-modal="true" aria-label={chrome.templates} onClick={(e) => e.stopPropagation()}>
            ...
            savedForms.map((f) => (
              <div key={f.id} style={styles.libraryRow}>
                ...
                <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                  <button style={styles.ghostBtn} disabled={busy} onClick={() => onOpen(f.id)}>
                    {busy && <Loader2 size={12} className="spin" />} {manage ? chrome.open : chrome.applyTemplate}
                  </button>
                  {manage && (
                    <button style={{ ...styles.iconBtn, ...styles.iconBtnDanger }} disabled={busy} title={chrome.delete} onClick={() => onDelete(f.id)}><Trash2 size={13} /></button>
                  )}
                </div>
              </div>
            ))
